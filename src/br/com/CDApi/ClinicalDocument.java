...
public class ClinicalDocument {
	 ...
	 * partir deste construtor é usada para acessar os métodos de escrita do Arquivo
	 * XML. <br>
	 * Exemplo de implementação:<br>
	 * <blockquote>
	 * 
	 ...
	 * <p>
	 * O método {@code getHeader} para a classe {@code ClinicalDocument} retorna um
	 * Header consistindo nos dados do cabeçario do qual o objeto é uma instância.
	 * Em outras palavras, este método deve retornar a informação contida no método
	...
	/**
	 * Recebe uma representação Header do objeto. O método {@code setHeader} para a
	 * classe {@code ClinicalDocument} deve receber as informações necessárias de um
	 * {@code Header} para que seja preenchido nos campos que corresponde aos dados
	 * de cabeçario no documento CDA. Para isto, as informações devem ser válidas. O
	 * método deve ser instanciado como mostrado na implementação: <blockquote>
	 * 
	 * <pre>
	 ...
	 * <p>
	 * O método {@code getPatient} para a classe {@code ClinicalDocument} retorna um
	 * Patient consistindo nos dados do paciente do qual o objeto é uma instância.
	 * Em outras palavras, este método deve retornar a informação contida no método
	 * instanciado por este objeto, como mostrado na implementação: <blockquote>
	 * 
	 * <pre>
	...
	/**
	 * Recebe uma representação Patient do objeto. O método {@code setPatient} para
	 * a classe {@code ClinicalDocument} deve receber as informações necessárias de
	 * um {@code Patient} para que seja preenchido nos campos que corresponde aos
	 * dados do paciente no documento CDA. Para isto, as informações devem ser
	 * válidas. O método deve ser instanciado como mostrado na implementação:
	 * <blockquote>
	 * 
	 * <pre>
	 ...
	 * <p>
	 * O método {@code getAuthor} para a classe {@code ClinicalDocument} retorna um
	 * Author consistindo nos dados do médico do qual o objeto é uma instância. Em
	 * outras palavras, este método deve retornar a informação contida no método
	 * instanciado por este objeto, como mostrado na implementação: <blockquote>
	 * 
	 * <pre>
	 * {@code
	...
	/**
	 * Recebe uma representação Author do objeto. O método {@code setAuthor} para a
	 * classe {@code ClinicalDocument} deve receber as informações necessárias de um
	 * {@code Author} para que seja preenchido nos campos que corresponde aos dados
	 * do médico no documento CDA. Para isto, as informações devem ser válidas. O
	 * método deve ser instanciado como mostrado na implementação: <blockquote>
	 * 
	 * <pre>
	 ...
	 * retorna um Authentic consistindo nos dados de autenticação do qual o objeto é
	 ...
	 * {@code setAuthenticator} para a classe {@code ClinicalDocument} deve receber
	 * as informações necessárias de um {@code Authentic} para que seja preenchido
	 * nos campos que corresponde aos dados de autenticação no documento CDA. Para
	 * isto, as informações devem ser válidas. O método deve ser instanciado como
	 ...
	 * <p>
	 * O método {@code getRelated} para a classe {@code ClinicalDocument} retorna um
	 * Related consistindo nos dados relacionados do qual o objeto é uma instância.
	 * Em outras palavras, este método deve retornar a informação contida no método
	 * instanciado por este objeto, como mostrado na implementação: <blockquote>
	 * 
	 * <pre>
	...
	/**
	 * Recebe uma representação Related do objeto. O método {@code setRelated} para
	 * a classe {@code ClinicalDocument} deve receber as informações necessárias de
	 * um {@code Related} para que seja preenchido nos campos que corresponde aos
	 * dados de relacionados no documento CDA. Para isto, as informações devem ser
	 * válidas. O método deve ser instanciado como mostrado na implementação:
	 * <blockquote>
	 * 
	 * <pre>
	 ...
	 * retorna um Responsible consistindo nos dados do responsável do paciente do
	 * qual o objeto é uma instância. Em outras palavras, este método deve retornar
	 * a informação contida no método instanciado por este objeto, como mostrado na
	 * implementação: <blockquote>
	 * 
	 * <pre>
	 * {@code
	 ...
	 * receber as informações necessárias de um {@code ResponsibleParty} para que
	 * seja preenchido nos campos que corresponde aos dados do responsável no
	 * documento CDA. Para isto, as informações devem ser válidas. O método deve ser
	 ...
	 * <p>
	 * O método {@code getComponents} para a classe {@code ClinicalDocument} retorna
	 * um ArrayList consistindo nos dados dos componentes que fazem parte do
	 * Documento. Em outras palavras, este método deve retornar um ArrayList
	 * contendo nas informaçoes do paciente, tais como: Historico medico,
	 * medicamentos, doenças, etc. Para exibir os dados dos componentes, o método
	 * deve ser instanciado como mostrado no exemplo de implementação: <blockquote>
	 * 
	 * <pre>
	 * {@code
	 ...
	 * <p>
	 * O método {@code getComponents} para a classe {@code ClinicalDocument} recebe
	 * um ArrayList consistindo nos dados dos componentes que fazem parte do
	 * Documento. Em outras palavras, este método deve receber um ArrayList contendo
	 * nas informaçoes do paciente, tais como: Historico medico, medicamentos,
	 * doenças, etc. Para exibir os dados dos componentes, o método deve ser
	 * instanciado como mostrado no exemplo de implementação: EXEMPLO PARA INSERIR
	 * HISTORICO DA DOENÇA DO PACIENTE <blockquote>
	 * 
	 * <pre>
	 * {@code
	 ...
	 * recebe um diretório como parâmetro onde será salvo o Arquivo XML gerado no
	 * local indicado. Retorna um valor booleano que indica o resultado da criação e
	 * validação de documento CDA. Esse documento sera Validado com o validador da
	 * própria aplicação, para que seus campos corresponda ao padrão HL7 CDA. O
	 * método deve ser instanciado como mostrado na implementação: <blockquote>
	 * 
	 * <pre>
	 ...
	 *
	 * @param path
	 *            local para onde sera escrito o arquivo XML
	...
	public boolean toGenerateCDAFile(String path) {
		...
		if (this.status == true) {
			System.err
					.println("Para gerar o arquivo o contrutor da classe ClinicalDocument não pode conter parâmetros!");
			return false;
		}
		toInitializeObjects();
		String idFile;
	 ...
	 * um documento CDA. Após o método ser chamado ele gera um documento CDA dentro
	 * da pasta XML, no diretório da aplicação, ao mesmo tempo que valida com o
	 * validador da própria aplicação, para que seus campos corresponda ao padrão
	 * HL7 CDA. O método deve ser instanciado como mostrado na implementação:
	 * <blockquote>
	 * 
	 * <pre>
